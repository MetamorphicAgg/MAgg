...
from trans import expand_sat_n_batch_cut_lvl2
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for prb in tqdm(xs):
            prbs_batches = expand_sat_n_batch_cut_lvl2(prb, n_samples=n_samples_lvl1, n_samples_2=n_samples_lvl2, n_cut=15)
    ...
    pickle.dump(preds, open('data/%s/%s_%d_rp_lv2_%d,%d_5000_preds.pkl' % (ver, ver, n_vars, n_samples_lvl1, n_samples_lvl2), 'wb'))
    pickle.dump(gts, open('data/%s/%s_%d_rp_lv2_%d,%d_5000_gts.pkl' % (ver, ver, n_vars, n_samples_lvl1, n_samples_lvl2), 'wb'))
