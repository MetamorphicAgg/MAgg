...
class InstanceLoader(object):
    ...
    def get_instances(self, n_samples=1024):
        res = []
        for f in tqdm(self.filenames[:n_samples]):
            # Read graph from file
            Ma,Mw,route = read_graph(f)
            res.append((Ma, Mw, route))
        return res

def create_batch(instances, route_exists, dev=0.02, target_cost=None):
    ...
    route_exists = np.array(np.array(route_exists))
    ...
    for (i,(Ma,Mw,route)) in enumerate(instances):
        ...
        if target_cost is None:
            C[m_acc:m_acc+m,0] = (1-dev)*cost if route_exists[i] == 0 else (1+dev)*cost
        else:
