    ...
    #with tf.Session() as sess:
        ...
        with open('training/dev={dev}/log.dat'.format(dev=dev),'a') as logfile:
            ...
            for epoch_i in np.arange(start_epoch, start_epoch + epochs_n):
                ...
                for (batch_i, batch) in tqdm(list(islice(enumerate(train_loader.get_batches(batch_size, dev)), train_params['batches_per_epoch']))):
                    train_stats['loss'][batch_i], train_stats['acc'][batch_i], train_stats['sat'][batch_i], train_stats['pred'][batch_i], train_stats['TP'][batch_i], train_stats['FP'][batch_i], train_stats['TN'][batch_i], train_stats['FN'][batch_i] = \
                        run_batch(sess, GNN, batch, batch_i, epoch_i, time_steps, train=True, verbose=True)
                summarize_epoch(epoch_i,train_stats['loss'],train_stats['acc'],train_stats['sat'],train_stats['pred'],train=True)

                print('Testing model...', flush=True)
                for (batch_i, batch) in tqdm(list(islice(enumerate(test_loader.get_batches(batch_size, dev)), test_params['batches_per_epoch']))):
                    test_stats['loss'][batch_i], test_stats['acc'][batch_i], test_stats['sat'][batch_i], test_stats['pred'][batch_i], test_stats['TP'][batch_i], test_stats['FP'][batch_i], test_stats['TN'][batch_i], test_stats['FN'][batch_i] = \
                        run_batch(sess, GNN, batch, batch_i, epoch_i, time_steps, train=False, verbose=True)
                summarize_epoch(epoch_i,test_stats['loss'],test_stats['acc'],test_stats['sat'],test_stats['pred'],train=False)
                test_acc = np.mean(test_stats['acc'])

                if epoch_i % 100 == 0: 
                    save_weights(sess, 'training/dev=%.2f/checkpoints/epoch=%d' % (dev, epoch_i))
